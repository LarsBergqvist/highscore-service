...
public class HighScoreRepository : IHighScoreRepository
{
    ...
    public async Task<IEnumerable<HighScoreListReadModel>> GetAllHighScoreLists()
    {
        try
        {
            ...
            return result.Select(res => res.ToReadModel()).ToList();
        }
        catch (Exception e)
        {
            _logger.LogError("{Message}",e.ToString());
    ...
    {
        if (!ObjectId.TryParse(highScoreListId, out var filterObjectId))
        {
            return null;
        }
        try
        {
            ...
            return result.Count == 0 ? null : result[0].ToReadModel();
        }
        catch (Exception e)
        {
            _logger.LogError("{Message}",e.ToString());
